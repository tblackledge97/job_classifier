...
class JobClassifier:
    ...
    def classify(self, job: str) -> dict:
        """Apply the above"""
        # Clean job titles
        clean_title = self.clean_title(job)

        # extract features
        seniority = self.seniority(clean_title)
        department = self.department(clean_title)
        function = self.function(clean_title)

        payload = {
            "seniority": seniority or "Unknown",
            "function": function or "Unkown",
